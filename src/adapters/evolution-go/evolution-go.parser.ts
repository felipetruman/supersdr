...
import type {
  ...
  EvoGoMessageUpsert,
  EvoGoWebhookPayload,
} from './evolution-go.schemas.js';

/**
 * Parser do Evolution Go.
 *
 * Como o Go usa whatsmeow (mesmo protocolo WA Multi-Device do Baileys),
 * a estrutura dos webhooks é praticamente idêntica ao evolution-baileys.
 * A diferença está no `provider` name na saída normalizada.
 */
...
function detectType(upsert: EvoGoMessageUpsert['data']): MessageType {
...
function extractText(upsert: EvoGoMessageUpsert['data']): string | undefined {
  const m = upsert.message;
...
function extractMedia(
  upsert: EvoGoMessageUpsert['data'],
...
function buildMessage(payload: EvoGoMessageUpsert): NormalizedMessage {
...
export function parseEvolutionGoPayload(
  payload: EvoGoWebhookPayload,
): NormalizedEvent[] {
