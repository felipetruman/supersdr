...
d2('WppClient — branches finais', () => {
  i2('image sem fileName/caption usa defaults', async () => {
    ...
    await new WC({ baseUrl: 'http://x', session: 's', token: 't', fetchImpl: f })
      .sendMessage('5547', { type: 'image', url: 'https://x/img' });
    const body = JSON.parse(((f as unknown as ReturnType<typeof v2.fn>).mock.calls[0][1] as RequestInit).body as string);
  ...
  i2('video sem fileName/caption usa defaults', async () => {
    const f = v2.fn().mockResolvedValue(okR({ id: 'x' })) as unknown as typeof fetch;
    await new WC({ baseUrl: 'http://x', session: 's', token: 't', fetchImpl: f })
      .sendMessage('5547', { type: 'video', url: 'https://x/v' });
    const body = JSON.parse(((f as unknown as ReturnType<typeof v2.fn>).mock.calls[0][1] as RequestInit).body as string);
  ...
  i2('location sem name/address usa defaults', async () => {
    const f = v2.fn().mockResolvedValue(okR({ id: 'x' })) as unknown as typeof fetch;
    await new WC({ baseUrl: 'http://x', session: 's', token: 't', fetchImpl: f })
      .sendMessage('5547', { type: 'location', latitude: -26, longitude: -49 });
    const body = JSON.parse(((f as unknown as ReturnType<typeof v2.fn>).mock.calls[0][1] as RequestInit).body as string);
    e2(body.title).toBe('');
  ...
  i2('phone com 16+ dígitos é tratado como grupo', async () => {
    const f = v2.fn().mockResolvedValue(okR({ id: 'x' })) as unknown as typeof fetch;
    await new WC({ baseUrl: 'http://x', session: 's', token: 't', fetchImpl: f })
      .sendMessage('1234567890123456', { type: 'text', text: 'oi' });
    const body = JSON.parse(((f as unknown as ReturnType<typeof v2.fn>).mock.calls[0][1] as RequestInit).body as string);
